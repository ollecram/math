...
In particolare, dato un qualunque insieme *finito* $E$ contenente $n$ elementi vi sono $n!$ distinte funzioni biunivoche definite su di esso:  Le funzioni contenute in $S$ sono dunque le *permutazioni* di $n$ elementi.. Ciascuna funzione $f_i$ definisce infatti una *corrispondenza* tra ciascuno degli $n$ elementi $x$ preso come *argomento* ed uno degli elementi $y = f_i(x)$ preso come *valore*. Per l'assunto di biunivocità l'insieme dei valori di una qualunque $f_i(x)$ coincide con $E$ ed esiste la funzione *inversa* $f^{-1}_i$ definita .  
...
$$ \sqrt{x^2} = x $$  
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0NDEyMzQyMDFdfQ==
-->